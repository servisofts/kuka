...
class SessionWeb {
    ...
    open() {
        ...
        Log("Itentando conectar con " + this.config["nombre"] + "@" + this.config.web["ip"] + " ::open");
        this.socket = new WebSocket(this.config.web["ip"]);
        this.socket.onopen = () => {
            _instance.onOpen();
        }

        this.socket.addEventListener('message', function (data) {
            _instance.onMensaje(data.data);
        });
    ...
    identificarse() {
        ...
        const reintent = async () => {
            ...
            if (!_instance.identificado) {
                var curTime = new Date().getTime();
                if (curTime - _instance.lastSendIdentificado > 1000) {
                    _instance.lastSendIdentificado = new Date().getTime();
                    var strUsuario = localStorage.getItem(AppParams.storage.urlLog);
                    ...
                    var token = "void";
                    var strFirebase = localStorage.getItem(AppParams.storage.fbtoken);
                    try {
                        ...
                        localStorage.setItem(AppParams.storage.fbtoken, JSON.stringify({
                            token: token,
                            type: "SS UUID"
                        }))
                    }
